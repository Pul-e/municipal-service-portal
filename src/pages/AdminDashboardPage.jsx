...
function AdminDashboardPage() {
      ...
      try {
        ...
        const mergedRequests = (requestsData || []).map((req) => {
          ...
          return {
            ...
            feedback_created_at: feedback?.created_at || null,
          };
        });
  ...
  const handleAssign = async (requestId, staffId) => {
    try {
      ...
      const staffName =
        assignedStaff?.full_name || assignedStaff?.email || 'Worker';
  ...
  return (
    <article className="page-container">
      ...
      {error && (
        <p className="error-message" style={{ margin: '1rem 2rem 0' }}>
          {error}
        </p>
      )}
      ...
      <section className="admin-stats" style={{ padding: '1.5rem 2rem' }}>
        ...
        <div className="stat-card">
          ...
          >
            {requests.length > 0
              ? Math.round((resolvedCount / requests.length) * 100)
              : 0}
            % rate
          </span>
        </div>
      ...
      <section className="dashboard-section" style={{ padding: '0 2rem 2rem' }}>
        ...
        >
          <div
            style={{
              display: 'grid',
              gridTemplateColumns: '2fr 3fr 1fr 1fr 1.5fr 0.7fr',
              padding: '10px 18px',
